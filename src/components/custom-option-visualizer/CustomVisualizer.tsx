...
async function draw_a_blank_plate_v2(
	itemOptions: ItemOptions,
	canvas_additive: Canvas,
	canvas_subtractive: Canvas,
	ctx: CanvasRenderingContext2D,
	text_width: number,
...
// export function draw_text_on_canvas(itemOptions: ItemOptions): number {
//   let canvas_additive: HTMLCanvasElement | null = null;
//   let canvas_subtractive: HTMLCanvasElement | null = null;

//   useVisibleTask$(() => {
//     canvas_additive = document.getElementById(ADDITIVE_PLATE_CANVAS_ID) as HTMLCanvasElement;
//     canvas_subtractive = document.getElementById(SUBTRACTIVE_PLATE_CANVAS_ID) as HTMLCanvasElement;

//     if (!canvas_additive || !canvas_subtractive) {
...
//     const boardWidth = text_width + BUILD_DIMS.ring_width + 2 * (BUILD_DIMS.text_margin + BUILD_DIMS.boarder_width);
//     const boardWidth_cm = (px2mm * boardWidth / 10).toFixed(1);
//     console.log(parseFloat(boardWidth_cm)); // Log the value instead of returning it
//   });

//   return 0; // Return a default value until the task runs
// }

export const BuildPlateVisualizer = component$((itemOptions: ItemOptions) => {
	const boardWidth_cm = useSignal<number>(0.0);
	const additivePlateImg = useSignal<string>('');
	const subtractivePlateImg = useSignal<string>('');

	const ADDITIVE_PLATE_CANVAS_ID = 'canvas_additive';
	const SUBTRACTIVE_PLATE_CANVAS_ID = 'canvas_subtractive';

	console.log('font_top_details:', itemOptions.font_top.value);

	useTask$(({ track }) => {
		// Track changes in itemOptions signals
		...
		console.log(
			'Drawing text on canvas with itemOptions:',
			itemOptions.text_top.value,
			itemOptions.text_bottom.value,
			itemOptions.font_top.value,
			itemOptions.font_bottom.value,
			itemOptions.primary_color.value,
			itemOptions.base_color.value
		);

		const canvas_additive = createCanvas(400, 100);
		const canvas_subtractive = createCanvas(400, 100);
		...
		additivePlateImg.value = canvas_additive.toDataURL('image/png');
		subtractivePlateImg.value = canvas_subtractive.toDataURL('image/png');
	});

	return (
		<div class="overflow-auto overflow-y-hidden p-0 bg-white">
			<div>
				<img src={additivePlateImg.value} alt="Additive Plate" class="w-400" />
				<img src={subtractivePlateImg.value} alt="Subtractive Plate" class="w-400" />
			</div>
			<div class="text-center text-sm text-gray-500 mt-2">
				Board width: {boardWidth_cm.value} cm
			</div>
		</div>
	);
});
