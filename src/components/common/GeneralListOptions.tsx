...
export default component$<GeneralListOptionsProps>(({ ListOptions, page, totalItems }) => {
	return (
		<div class="max-w-6xl m-auto rounded-lg p-4 space-y-4">
			...
			<div class="flex justify-center items-center gap-3">
				...
				<span class="text-sm">
					Page {page.value}
					{totalItems.value ? ` of ${Math.max(1, Math.ceil(totalItems.value / PAGE_SIZE))}` : ''}
				</span>
				<button
					class="px-3 py-1 rounded border"
					...
					disabled={
						...
						page.value >= Math.max(1, Math.ceil(totalItems.value / PAGE_SIZE))
					}
				>
					<LuMoveRight />
				</button>
